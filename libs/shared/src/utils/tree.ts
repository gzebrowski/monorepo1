...
export class TreeManager<T extends TreeModel> {
  ...
  async addChild(relativeObj: T, newRecordData: Omit<T, 'id' | 'path' | 'depth'>): Promise<T> {
    if (!relativeObj.path || relativeObj.depth === null) {
      throw new Error('Parent node must have a valid path and depth');
    }

    // Find the last child of the parent
    const lastChild = await this.queryset.findFirst({
      where: {
        path: {
          startsWith: relativeObj.path,
          not: relativeObj.path
        },
    ...
    if (!lastChild || !lastChild.path) {
      // First child
      newPath = relativeObj.path + this.intToBase36(1).padStart(this.steplen, '0');
    } else {
      // Get the child number from the last child's path
      const childPathPart = lastChild.path.substring(relativeObj.path.length, relativeObj.path.length + this.steplen);
      const lastChildNum = this.base36ToInt(childPathPart);
      newPath = relativeObj.path + this.intToBase36(lastChildNum + 1).padStart(this.steplen, '0');
    }
  ...
  async addSibling(
    ...
    if (!relativeObj.path || relativeObj.depth === null) {
      throw new Error('Reference node must have a valid path and depth');
    }
    ...
    switch (position) {
      case 'first-sibling':
        return this.insertSiblingAt(parentPath, relativeObj.depth, newRecordData, 'first');
      
      case 'last-sibling':
        return this.insertSiblingAt(parentPath, relativeObj.depth, newRecordData, 'last');
      
      case 'left':
        return this.insertSiblingBefore(relativeObj, newRecordData);
      
      case 'right':
        return this.insertSiblingAfter(relativeObj, newRecordData);
  ...
  async move(nodeToMove: T, targetObj: T, position: Position): Promise<T> {
    ...
    switch (position) {
      case 'first-child':
      case 'last-child':
        newPath = await this.calculateChildPath(targetObj, position === 'first-child');
      ...
      case 'left':
      case 'right':
        const siblingData = await this.calculateSiblingPath(targetObj, position);
        newPath = siblingData.path;
        newDepth = siblingData.depth;
        break;
    ...
    for (const descendant of descendants) {
      if (!descendant.path || descendant.depth === null) {
        throw new Error('Descendant must have valid path and depth');
      }
  ...
  async rebuildTreeFromRelationships(relationships: Array<{id: string, parentId: string | null, [key: string]: any}>): Promise<void> {
    // Clear existing path and depth values
    ...
    const itemMap = new Map(relationships.map(item => [item.id, item]));
    const childrenMap = new Map<string, typeof relationships>();
    
    // Find roots and build children map
    const roots = relationships.filter(item => !item.parentId);
    ...
    for (const root of roots) {
      ...
      await this.rebuildTreeRecursive(root.id, rootPath, 1, childrenMap);
      rootCounter++;
    }
  }
  ...
  private async insertSiblingAt(
    ...
    const existingSibling = await this.queryset.findFirst({
      where: whereClause,
    ...
    if (!existingSibling) {
      // No siblings exist
      newPath = (parentPath || '') + this.intToBase36(1).padStart(this.steplen, '0');
    } else {
      const siblingPathPart = existingSibling.path.substring(parentPath?.length || 0, (parentPath?.length || 0) + this.steplen);
      const siblingNum = this.base36ToInt(siblingPathPart);
      
      if (position === 'first') {
        // Insert before the first sibling - this requires shifting existing nodes
        await this.shiftSiblingsRight(parentPath, depth);
        newPath = (parentPath || '') + this.intToBase36(1).padStart(this.steplen, '0');
      } else {
        // Insert after the last sibling
        newPath = (parentPath || '') + this.intToBase36(siblingNum + 1).padStart(this.steplen, '0');
      }
    }
  ...
  private async insertSiblingBefore(relativeObj: T, newRecordData: Omit<T, 'id' | 'path' | 'depth'>): Promise<T> {
    ...
    // you might want to implement gap management for better performance
    const parentPath = this.getParentPath(relativeObj.path!);
    ...
    if (siblingNum === 1) {
      // Insert at the beginning, need to shift others
      await this.shiftSiblingsRight(parentPath, relativeObj.depth!);
      const newPath = (parentPath || '') + this.intToBase36(1).padStart(this.steplen, '0');
      return await this.queryset.create({
        data: {
          ...newRecordData,
          path: newPath,
      ...
      const newPath = (parentPath || '') + this.intToBase36(siblingNum - 1).padStart(this.steplen, '0');
      return await this.queryset.create({
        data: {
          ...newRecordData,
          path: newPath,
  ...
  private async calculateSiblingPath(target: T, position: string): Promise<{path: string, depth: number}> {
    ...
    switch (position) {
      case 'first-sibling':
        return { 
          path: (parentPath || '') + this.intToBase36(1).padStart(this.steplen, '0'),
          depth: target.depth!
        };
      case 'last-sibling':
        ...
        return {
          path: (parentPath || '') + this.intToBase36(lastNum + 1).padStart(this.steplen, '0'),
          depth: target.depth!
        };
      default:
        throw new Error(`Invalid sibling position: ${position}`);
    }
  }
