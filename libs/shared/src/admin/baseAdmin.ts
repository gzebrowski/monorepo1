...
export type FieldConfig = {
    widget?: 'input' | 'textarea' | 'select' | 'checkbox' | 'date';
    pattern?: string;
    max?: number;
    min?: number;
};
...
export class BaseAdminModel {
    ...
    parseDefaultValue(fieldMetadata: FieldDefinition) {
        /*
        */
    }
    ...
    async validateCreateData(data: Record<string, any>, fields?: string[], exclude?: string[]): Promise<Record<string, any>> {
        const fieldsAndTypes = await this.getPrismaModelFieldsAndTypes();
        const extraFields = await this.getExtraFields();
        const allExtraFieldNames = extraFields.map(field => field.field);
        const fieldToMetadata: Record<string, FieldDefinition> = {};
        ...
        for (const key in data) {
            ...
            if (itemMetadata.data_type === 'date' || itemMetadata.data_type === 'datetime' || itemMetadata.data_type.startsWith('timestamp')) {
                if (typeof data[key] === 'string') {
                    try {
                        if (itemMetadata.data_type === 'date') {
                ...
                if (newData[key] && !(newData[key] instanceof Date)) {
                    console.error(`Error 2!!!!: Invalid date format for field ${key}`, newData[key], typeof newData[key]);
    ...
    async create(data: Record<string, any>, exclude?: string[], fields?: string[]) {
        ...
        const validatedData = await this.prepareCreateData(await this._validateData(await this.validateCreateData(data, fields, allExcludeFields)));
    ...
    async getInlineItems(inlines: InlineDefinition[], idItem: string, relationModelsToAdminInstanceMap: RelationModelsToAdminInstanceMap) {
        ...
        const relationsToInline = inlines.filter(inline => possibleModelsForInlines.includes(inline.model));
    ...
    async saveInlines(idItem: string, existingItems: Record<string, ExistingFormData[]>, newItems: Record<string, NewFormData[]>, adminMap: RelationModelsToAdminInstanceMap) {
        const myModel = this.getPrismaModel(false);
        await Promise.all(Object.keys(existingItems).map(async (model) => {
            ...
            await Promise.all(existingItems[model].map(async (existingItem) => {
                const data = existingItem.formData;
                if (existingItem.id) {
                    await relAdminInstance.update(existingItem.id, data);
                }
            }));
        }));
        await Promise.all(Object.keys(newItems).map(async (model) => {
            ...
            const relAdminInstance = adminMap[model].adminModel;
            await Promise.all(newItems[model].map(async (newItem) => {
